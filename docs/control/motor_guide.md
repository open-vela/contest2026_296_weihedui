...
| 功能 | GPIO引脚 | 说明 |
|------|----------|------|
| 开门电机 | 0（占位，待接线后填入） | 高电平触发 |
| 关门电机 | 0（占位，待接线后填入） | 高电平触发 |
| 锁门电机 | 0（占位，待接线后填入） | 高电平触发 |
| 蜂鸣器 | 0（占位，待接线后填入） | 高电平触发 |

> **2026-09-19 实机修正**：上表此前的 `GPIO 26/27/28/29` 是**设计期示例值，与实际代码不符**。
> `src/smart_lock_main.c` 中 `g_default_motor_config` 的四个引脚号**均为占位值 `0`**；
> 且 `src/door_control.c` 的 `gpio_write()` 目前是**只打印日志的桩函数**
> （该文件顶部标注为「GPIO 模拟定义」），不会驱动真实引脚。
> 因此电机状态机、超时保护、防夹联动逻辑可在实机上完整演示与验证，
> 但**不产生真实物理动作**。详见 `README.md` 第六节第 1 条。
